...
import static com.soufianekre.uquick.helpers.FirebaseConstant.DRIVER_WORKING_PATH;
...
public class DriverMapPresenter<V extends DriverMapMvp.View> extends BasePresenter<V> implements DriverMapMvp.Presenter<V> {
    ...
    public void findCustomers(){
        ...
        Executors.newSingleThreadExecutor().execute(() ->{
            getAssignedCustomer(driverLastLocationLatLng);
        });
    }


    private void getAssignedCustomer(LatLng driverLastLocationLatLng){
    ...
    public void disconnectDriver() {
        ...
        if (FirebaseAuth.getInstance().getCurrentUser() != null){
            ...
            geoFire.removeLocation(userId, (key, error) -> {
                getMvpView().showMessage("Driver Disconnected");
            });
        }
    }
